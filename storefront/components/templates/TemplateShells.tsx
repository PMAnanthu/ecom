...
// Get the store base path — /s/storename if on that route, else empty
    ...
    <div className="flex min-h-screen bg-neutral-50">
      <aside className="hidden lg:flex w-60 min-h-screen bg-white border-r flex-col shrink-0">
        <div className="p-5 border-b">
          <Link href={`${base}/`} className="text-lg font-bold block">{store?.name || 'Shop'}</Link>
        </div>
        <nav className="flex-1 p-4 flex flex-col gap-1 text-sm">
          {navLinks.map(l => <Link key={l.href} href={l.href} className="py-2 px-3 rounded hover:bg-neutral-100 text-neutral-700 font-medium">{l.label}</Link>)}
          {sidebarContent && <div className="mt-4 border-t pt-4">{sidebarContent}</div>}
          <Link href={`${base}/cart`} className="flex items-center justify-between py-2 px-3 rounded bg-black text-white hover:bg-neutral-800 mt-2">
            <span>🛒 Cart</span>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="lg:hidden sticky top-0 z-40 bg-white border-b flex items-center justify-between px-4 h-14">
          <Link href={`${base}/`} className="font-bold text-base truncate max-w-[180px]">{store?.name || 'Shop'}</Link>
        ...
        <MobileMenu open={menuOpen} onClose={() => setMenuOpen(false)} links={navLinks} cartCount={cartCount} user={user} onLogout={handleLogout} />
    ...
    <div className="min-h-screen bg-white">
      <header className="sticky top-0 z-50 bg-white border-b shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center gap-4">
          <Link href={`${base}/`} className="font-bold text-lg shrink-0 truncate max-w-[150px] sm:max-w-none">{store?.name || 'Shop'}</Link>
          <nav className="hidden md:flex items-center gap-1 flex-1 text-sm">
            {navLinks.map(l => <Link key={l.href} href={l.href} className="px-3 py-1.5 rounded hover:bg-neutral-100 text-neutral-700">{l.label}</Link>)}
          </nav>
          <div className="hidden md:flex items-center gap-3 text-sm shrink-0">
            {user ? <button onClick={handleLogout} className="text-neutral-500 hover:text-black text-xs">Logout</button>
              : <Link href={`${base}/login`} className="text-neutral-600 hover:text-black">Sign in</Link>}
            <Link href={`${base}/cart`} className="flex items-center gap-1 bg-black text-white px-3 py-1.5 rounded-full text-sm hover:bg-neutral-800">
    ...
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg,#f5f7fa 0%,#e8ecf1 100%)' }}>
      <header className="bg-white/80 backdrop-blur border-b sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center gap-4">
          <Link href={`${base}/`} className="font-extrabold text-lg tracking-tight shrink-0 truncate max-w-[150px] sm:max-w-none">{store?.name || 'Shop'}</Link>
          <nav className="hidden md:flex items-center gap-1 flex-1 text-sm">
            {navLinks.map(l => <Link key={l.href} href={l.href} className="px-3 py-1.5 rounded-full hover:bg-neutral-100 text-neutral-700">{l.label}</Link>)}
          </nav>
          <div className="hidden md:flex items-center gap-3 text-sm shrink-0">
            {user ? <button onClick={handleLogout} className="text-neutral-500 hover:text-black text-xs">Logout</button>
              : <Link href={`${base}/login`} className="text-neutral-600 hover:text-black">Sign in</Link>}
            <Link href={`${base}/cart`} className="flex items-center gap-1 bg-indigo-600 text-white px-4 py-1.5 rounded-full hover:bg-indigo-700">
