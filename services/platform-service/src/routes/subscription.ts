...
const schema = z.object({
  ...
  billingPeriod: z.enum(['MONTHLY', 'QUARTERLY', 'YEARLY', 'UNLIMITED']).default('MONTHLY'),
...
subscriptionRouter.patch('/:id', async (req: Request, res: Response) => {
  const updateSchema = z.object({
    name: z.string().optional(),
    price: z.number().min(0).optional(),
    currency: z.string().optional(),
    billingPeriod: z.enum(['MONTHLY', 'QUARTERLY', 'YEARLY', 'UNLIMITED']).optional(),
  });
  const parsed = updateSchema.safeParse(req.body);
  if (!parsed.success) { res.status(400).json({ error: parsed.error.flatten() }); return; }
  const sub = await prisma.subscription.update({ where: { id: req.params.id }, data: parsed.data });
